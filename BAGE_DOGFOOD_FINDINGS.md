...
| 7 | `bage create`/`apply` parse floor **accepts syntactically broken content** (Go `func f( {` was written, exit 0). tree-sitter is error-tolerant — it returns a tree with ERROR nodes rather than failing, so the floor ("did it produce a tree?") always passes. | E2E diagnose run. | **OPEN (design)** | The floor rejects only catastrophic parse failure, not ERROR/MISSING nodes. Two readings: (a) intended — an agent IDE must allow broken intermediate states, and `diagnose` *does* surface the ERROR node while Hylla owns project correctness; (b) gap — the ADR's "would the staged bytes still parse → hard reject if not" implies stricter rejection. Decide strict-vs-lenient against Hylla's needs before changing. |
| 8 | `bage delete` left the now-empty parent dir (`diag/`) on disk; the ADR file-lifecycle section says delete reaps empty dirs. | E2E delete run. | **OPEN** | Minor: either implement empty-parent reaping in `DeleteFile` or amend the ADR to drop the claim. Low priority. |
