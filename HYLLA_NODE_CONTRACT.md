...
| Field | Type | Meaning |
|---|---|---|
| `name` | string | exported symbol — a re-export LEAF (`*` glob re-export, `default` default export); brace-group items are FLATTENED to the leaf name, the intermediate path folded into `re_exported_from` (BF4) |
...
| `re_exported_from` | string? | source module when the export forwards another module's symbol (Rust `pub use`, TS `export … from`) — a brace-group's shared path plus the item's intermediate segments (`pub use crate::{a::X}` → `crate::a`, BF4) |
| `alias` | string? | exported-as name when renamed (`export { x as y }`, `export * as ns from` → `ns`) |
| `glob` | bool | wildcard re-export whose members are UNKNOWN (`pub use a::*`, nested-in-group `{a::*}`, `export * from`, `export * as ns from`); `name` == `*`. A glob row beside a same-name own decl makes the facade AMBIGUOUS (BF4) |
| `visibility` | `ExportVisibility` (enum, snake_case) | `public` = a crate-external `pub` (every own decl, every non-Rust export); `restricted` = a bounded Rust `pub(crate)`/`pub(super)`/`pub(in path)` RE-EXPORT — emitted (existence only) so a consumer can detect a bounded re-export coexisting with a same-name own decl (BF4) |
...
| Re-export (export side) | `pub use` → `ExportFact{reexport, re_exported_from}`; brace-group items FLATTENED to leaf (`{a::X}`→ leaf `X`, `re_exported_from=crate::a`); a **bare-module** re-export (`pub use util;`) → row `name=util`, `re_exported_from=util` (BF4); a **`{self}`** leaf (`pub use a::b::{self}`) → the MODULE leaf `b` from `a` (NEVER literal `self`; `{self as r}` keeps `alias=r`) (BF4); **`pub extern crate foo [as bar]`** → `reexport` row `name=foo`/`re_exported_from=foo`/`alias=bar` (BF4); glob → `glob=true` `*` row (`a::*`, nested `{b::*}`); **restricted-vis re-export** (`pub(crate)`/`pub(super)`/`pub(in …)` use OR `extern crate`) EMITTED as `visibility=restricted` (BF4) | `export {…} from`, `export * from` (`glob=true`), `export * as ns from` (`glob=true`, `alias=ns`) | — (no export grammar; module-level re-binds surface as `ImportFact`s) | — (no export grammar) |
| Default export | — | `export default …` | — | — |

Rust restricted visibilities (`pub(crate)`, `pub(super)`, `pub(in path)`) are NOT crate-external exports. Own DECLS at a restricted visibility are NOT emitted (only a bare `pub` own decl exports); a restricted RE-EXPORT (`pub(crate) use …`) IS emitted with `visibility=restricted` (BF4) so a consumer can spot own-decl-vs-reexport coexistence — a plain visibility-less `use` stays a private import (no export row). Python/Go have no syntactic export construct, so the three re-export classes (restricted-vis / brace-group / glob) do not arise on an export leg there — Python re-exports surface as `ImportFact`s. Fact PAYLOADS (`ImportFact`/`ExportFact`/`ImportedItem`/`ExportKind`/`ExportVisibility`) are serde-friendly for golden fixtures; the outer `Facts` result carries a typed `Lang` and is a live branch, not a wire type.

**Known export-leg limitations (BF4, accepted).** These are OVER-emission or scope-precision gaps whose error direction is always *false-ambiguity*, never *false-uniqueness* (the harm the export leg exists to prevent), so they are documented rather than fixed: (1) **enclosing-module scope not carried** — a `pub use` inside a private `mod {}` is emitted `visibility=public` and file-wide (no enclosing-`mod` qualifier), though its effective visibility is bounded by the private module; a consumer sees an EXTRA re-export row, never a missing one. (2) **`pub use X as _;`** — an anonymous re-export (trait-impl-only, no exported NAME) emits `name=X`, `alias=_`; a consumer treating `_` as a real binding mints a phantom `_` — treat `alias == "_"` as anonymous (no named export). (3) **macro-generated exports are invisible** — a `pub use`/item synthesized by a `macro_rules!` invocation is not expanded by the static parser and emits no row; this is the inherent static-analysis floor, not a BF4 regression. Carrying enclosing-module scope on `ExportFact` (fixing #1, and the nested-facade case) is a roadmap item.
...
**IMPORT PLACEMENT SCOPING (BF2):** an import/use statement is a BINDING SITE for its local names in the scope it lexically SITS IN, resolved by walking the scope chain — NOT one file-wide fallback set. A TOP-LEVEL `use`/`import` binds into the module root, so most occurrences' chains reach it (visible file-wide, as before). A **fn-body / block-scoped import** (Rust fn-body `use crate::x::h;`, Python `def g(): from m import h`) binds ONLY into that function/block scope, so its names are visible solely down that subtree: a nested function/block inside it sees the name (`ImportedName`), but a **SIBLING function that cannot reach the declaration resolves `Undecidable`** — never a false file-wide `ImportedName`. `resolve_name` checks each scope's `import_bindings` AFTER that scope's local bindings (a same-scope local still shadows the import), inner-to-outer. `Scope.import_bindings` is resolution-only, `#[serde(skip)]` — NOT part of the wire contract.

**Rust `mod` boundary (E0425):** a child module does NOT inherit its parent file/mod `use` bindings — a bare name inside `mod inner` needs its own `use` or a `super::`/`crate::` path. So `resolve_name` STOPS consulting `import_bindings` once the chain walk climbs PAST a nested Rust `mod` scope (a `Module` with a parent): a top-level `use helper;` colliding with a bare `helper()` inside a nested `mod` now resolves `Undecidable`, never a false file-wide `ImportedName`. The nested mod's OWN `use` (written inside it) stays visible — the cut-off flips only AFTER that scope is checked. The gate is import-only (local bindings/poison unaffected), matching the narrow E0425 truth.

**Python bare dotted import binds the HEAD (`import os.path` → `os`):** Python's `import a.b.c` (no alias) binds ONLY the head package name `a` locally — `a.b.c` is then reached via attribute access, and a bare `path` from `import os.path` is a real `NameError`. `import_local_names` binds the dotted head for Python bare imports (tail path segment for every other language's bare import, e.g. Go `import "a/b/c"` → `c`). The ALIASED form `import a.b as x` (non-empty `items`) and `from a.b import c` are unchanged (alias / imported item names).

**ACCEPTED GAP — Python body-import use-before-import:** `import_bindings` is a positionless `HashSet`, so a Python body import is visible to a use LEXICALLY BEFORE it in the same scope (`def g(): helper(); from m import helper` → `ImportedName`), whereas the runtime raises `UnboundLocalError`. This is CORRECT for Rust/TS (non-positional / hoisted import semantics) and a documented-accepted false-definite gap for Python body imports only (a rare pre-import use); the F5 byte-gate that governs sequential `let`/`:=` locals is NOT applied to import bindings. Same-scope ordering the OTHER way — a local declared BEFORE a same-name body import (`helper = 1; from m import helper; return helper`) — reports `LocalBinding` (the local shadows the import in `import_bindings`-after-`bindings` order); the variable identity is right, but the kind label is approximate for that ordering (Python's later import rebinds the same slot).
...
Occurrences are all `identifier` nodes (binding sites included — a binding site resolves to its own `LocalBinding`), EXCEPT **non-referencing positions that are DROPPED so they never coerce a false `ImportedName`** (the XH3 false-definite class; only a WHOLE bare callee/use stays). SIX drop classes, all four grammars (the F4 attribute-drop precedent extended): (1) **import/use span** — ANY identifier anywhere inside an import/use/from-import declaration (Rust `use_declaration` incl. a fn-body-scoped `use crate::x::h;` AND `extern_crate_declaration`, TS/JS `import_statement` PLUS a re-export-`from` specifier/`namespace_export` name — an `export_statement` with a `source` field, sourceless local `export {x}` stays, Python `import_statement`/`import_from_statement`/`future_import_statement`, Go `import_declaration`/`import_spec`): an import path/name/alias identifier is a binding-site, never a body use; (2) **qualified-path TRAILING segment** — a Rust `scoped_identifier` `name` field (`crate::internal::helper()` → `internal`,`helper`) and a Python `dotted_name` segment (`import a.b.c`), while the path HEAD stays a real reference (Rust `T::default()`/`a::b` `path` field = generic param / module; Python attribute-chain head `a` in `a.b.c`); (3) Python `attribute` field (`obj.json` → `json`), (4) `keyword_argument` name (`f(json=1)` → `json`), and `global`/`nonlocal` declaration names; (5) **TSX intrinsic tag** — a lowercase JSX element name (`<div>`/`</div>`) is an HTML/SVG intrinsic, not a value reference (an uppercase component tag `<Foo/>` IS a reference and stays), and **TS index-signature parameter** — `[key: string]: T` binds no value; (6) **DEFINITION NAME site (BF3)** — the `name` field of an ENUMERATED definition-kind node is a declaration SITE, never a resolvable use, so it is dropped. The guarantee is scoped to EXACTLY the kinds listed here (NOT to every definition-shaped node), and this enumeration matches the `is_definition_name` code table one-for-one: Rust `function_item`/`function_signature_item`/`struct_item`/`enum_item`/`union_item`/`trait_item`/`const_item`/`static_item`/`type_item`/`mod_item`/`macro_definition`/`enum_variant`/`field_declaration` names (incl. assoc `fn`/`const` names in an `impl`/`trait` body); TS/JS `function_declaration`/`generator_function_declaration`/`function_expression`/`generator_function`/`function_signature` (ambient `declare function` + overload signatures)/`class_declaration`/`abstract_class_declaration`/`method_definition`/`method_signature`/`abstract_method_signature`/`public_field_definition`/`property_signature`/`enum_declaration`/`interface_declaration`/`type_alias_declaration`/`internal_module` (`namespace`)/`module` names; Python `function_definition`/`class_definition` names (methods included); Go `function_declaration`/`method_declaration`/`type_spec`/`field_declaration` names PLUS a func-TYPE `parameter_declaration` name whose immediate enclosing owner is a `function_type` (`type F func(helper int)` — no scope, no binding; a REAL func/method/`func_literal` parameter binds `LocalBinding` and stays). A definition name is ALREADY a binding (via `decl_name_binding`/`collect_*`, run off the PARENT node) — dropping the identifier from the OCCURRENCE stream never perturbs binding collection; it only prevents a definition name colliding with an imported name (`use origin::helper;` + `fn helper()`) from resolving a FALSE `ImportedName` and emitting a phantom `Extracted` edge. A body USE of the same spelling is not a `name` field and stays. Go qualified access (`fmt.Println`) uses a `selector_expression` whose `field` is a `field_identifier` (never an occurrence); its `operand` head (`fmt`) is a real bare use and stays. A Rust `if let` binding reaches only the consequence block — the `else`/`else if` alternative is walked under the enclosing scope, so the pattern name does NOT leak into the else arm. A Python `class` body opens a scope whose bindings do NOT enter a nested method's chain: a method's `Function` scope is re-parented to the nearest NON-class ancestor (module / enclosing function), so class vars need `self.`/`ClassName.` and never resolve `LocalBinding` inside a method (G5). All `Scopes` payloads (`ScopeTree`/`Scope`/`Binding`/`Occurrence`/`ScopeKind`/`Resolution`) are serde-friendly for golden fixtures; the outer `Scopes` result carries a typed `Lang` and is a live branch, not a wire type. `Scope.poison` (the G7 backstop) is resolution-only, `#[serde(skip)]` — NOT part of the wire contract.
...
| Non-referencing positions dropped (occurrence-stream purity, XH3) | import/use span; `scoped_identifier` trailing `name` (path head stays); field access = `field_identifier` (n/a) | import span; TSX intrinsic (lowercase) tag; `index_signature` param name; member = `property_identifier` (n/a) | import span; `dotted_name` segment (attribute-chain head stays; `case a.b:` pattern head conservatively dropped); `attribute` field, `keyword_argument` name, `global`/`nonlocal` names | import span; selector `field` = `field_identifier`, operand head stays (n/a otherwise) |
...
| NON-BINDING | `send_statement`; `labeled_statement`; `receive_statement` (`=` reassign) | `send_statement`: `field channel/value -> [_expression]` (both uses). `labeled_statement`: `field label -> [label_name]` (jump target, not a value). **XH3 occurrence-drop:** identifiers inside `import_declaration`/`import_spec` are NOT occurrences (import path/alias = binding-site); a `selector_expression` `field` is a `field_identifier` (never an occurrence), its `operand` head stays. |
...
| NON-BINDING | `labeled_statement` | `field label -> [statement_identifier]` (jump target, not a value binding). **XH3 occurrence-drop:** identifiers inside `import_statement` are NOT occurrences (import binding-site); a re-export-`from` specifier / `namespace_export` name (an `export_statement` carrying a `source` field — `export {x} from './m'`, `export * as ns from './m'`) names another module's binding and is dropped, while a SOURCELESS `export {x}` (local re-export) STAYS; a lowercase JSX intrinsic tag name (`jsx_opening_element`/`jsx_closing_element`/`jsx_self_closing_element` `name` field, `<div>`) and a TS `index_signature` `name` (`[key: string]`) are dropped — an uppercase component tag (`<Foo/>`) stays. |
...
| NON-BINDING | `global_statement`/`nonlocal_statement`; `attribute`/`subscript` targets | `global`/`nonlocal`: `children [identifier]` names DROPPED (F6). `obj.x += 1` / `d[k] += 1`: attribute/subscript targets introduce no local name (H2 collector skips). **XH3 occurrence-drop:** identifiers inside `import_statement`/`import_from_statement` and every `dotted_name` segment (`import a.b.c`) are NOT occurrences; the ATTRIBUTE-CHAIN head (`a` in an `attribute` `a.b.c`) stays a real use. CAVEAT: a `match`/`case a.b:` value-pattern parses the WHOLE dotted path as `dotted_name`, so its head `a` is dropped too — CONSERVATIVE over-drop (no false definite; head-stays applies to attribute chains only, not case patterns). |
...
| NON-BINDING | `const_item`/`static_item` value; lifetime generic params (`<'a>`) | `const_item`: `field value -> [_expression]` (RHS = use). `lifetime_parameter` USES are `lifetime` tokens, NEVER recorded as `identifier` occurrences → NO collision surface. **XH3 occurrence-drop:** identifiers inside a `use_declaration` (incl. fn-body `use crate::x::h;`), an `extern_crate_declaration` (`extern crate alloc;` / `… as a;` — a crate-binding site), and the trailing `name` segment of a `scoped_identifier` (`crate::internal::helper()` → `internal`,`helper`) are NOT occurrences; the path HEAD (`path` field — `T::default()`/`a::b`) stays a real reference. |
